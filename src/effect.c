...
void effect_box_shrink_grow (int iterations, int rot, int flip, uint16_t delay)
{
	...
	for (x=0;x<iterations;x++)
	{
		for (i=0;i<16;i++)
		{
            ...
            __disable_irq(); // disable interrupts while the cube is being rotated
...
void boingboing(uint16_t iterations, int delay, unsigned char mode, unsigned char drawmode)
{
	...
	unsigned char crash_x, crash_y, crash_z;
		int snake[8][3]; // Coordinate array for the snake.
...
void effect_random_sparkle (void)
{
	...
	for (i=1;i<20;i++)
	{
		effect_random_sparkle_flash(5,i,15);
	}
...
void random_pixels(int delay,  int iterations)
{
...
void tube_cube(int delay, int iterations) {
	int i, j;
	int x, y, z;
	for (i = 0; i < iterations; i++) {
