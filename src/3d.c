...
void linespin(int iterations, int delay) {
	...
	for (i = 0; i < iterations; i++) {
		...
		for (z = 0; z < 8; z++) {
			...
			line_3d((int) top_z, (int) top_x, (int) top_y, (int) bot_z,
					(int) bot_x, (int) bot_y);
			//tmp2cube();
		}
		tmp2cube();
...
void sinelines(int iterations, int delay) {
	...
	for (i = 0; i < iterations; i++) {
		for (x = 0; x < 8; x++) {
			...
			left = 4 + tan(sine_base) * ripple_height;
	...
	for (i = 0; i < iterations; i++) {
		for (x = 0; x < 8; x++) {
			for (y = 0; y < 8; y++) {
				...
				ripple_interval = 1.3;
				height = 4
						+ sin(distance / ripple_interval + (float) i / 50) * 4;

				tmpsetvoxel(x, y, (int) height);
			}
		}
		tmp2cube();
		delay_ms(delay);
	...
	for (i = 0; i < iterations; i++) {
		...
		for (x = 0; x < 8; x++) {
			for (y = 0; y < 8; y++) {
				...
				ripple_interval = 2;
				height = 4
						+ sin(distance / ripple_interval + (float) i / 50)
								* 3.6;
...
void fireworks(int iterations, int n, int delay) {
	...
	for (i = 0; i < iterations; i++) {
		...
		for (e = 0; e < origin_z; e++) {
			tmpsetvoxel(origin_x, origin_y, e);
			delay_ms(100 + 10 * e);
			//delay_ms(600+500*e);
			tmp2cube();
			tmpfill(0x00);
...
//		// explode
		for (e = 0; e < 25; e++) {
			...
			for (f = 0; f < n; f++) {
				...
				tmpsetvoxel(particles[f][0], particles[f][1], particles[f][2]);
...
void effect_rotate_random_pixels(int iterations, int delay, int pixels) {
	...
	for (i = 0; i < iterations; i++) {
		...
		for (p = 0; p < pixels; p++) {
			rotated[p] = point_rotate_around_point(points[p], cube_center,
					rot_x, rot_y, rot_z);
		}

		fill(0x00);
		for (p = 0; p < pixels; p++) {
			setvoxel((int) rotated[p].x, (int) rotated[p].y,
					(int) rotated[p].z);
		}
